...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var iconthing:FlxSprite = new FlxSprite(100, 0);
		iconthing.scale.x = scale;
		iconthing.scale.y = scale;
		iconthing.frames = Paths.getSparrowAtlas('meun_idle');
		iconthing.animation.addByPrefix('idle', 'idle', 24);
		iconthing.animation.play('idle');
		add(iconthing);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 6:
					#if PSYCH_WATERMARKS
					createCoolText(['lemmeo'], -40);
					#else
					createCoolText(['lemmeo'], -40);
					#end
				case 8:
					addMoreText('lemmeo', -40);
					ngSpr.visible = true;
				// credTextShit.text += '\nNewgrounds';
				case 9:
					deleteCoolText();
