...
class HorzFuzzShader extends FlxShader
{
	@:glFragmentSource('
	#pragma header
...
class HorzFuzzEffect extends Effect
{
	public var shader:HorzFuzzShader;
    public function new(horz:Float = 0.00){
    shader = new HorzFuzzShader();
      shader.data.horzFuzzOpt.value = [horz];
    }
...
class ChromaticAberrationEffect extends Effect
{
    ...
    public function new(popopo:Float = 0.00){
    shader = new ChromaticAberrationShader();
      shader.data.rOffset.value = [popopo];
      shader.data.bOffset.value = [-popopo];
    }
...
class GlitchaShader extends FlxShader{
	@:glFragmentSource('
...
class GlitchaEffect extends Effect{
	public var shader:GlitchaShader = new GlitchaShader();
    public function new(guh:Float){
      shader.prob.value = [guh];
    }
}
...
class PinCushionShader extends FlxShader{
    ...
    #define DISTORTION_AMOUNT ((distort * 0.8) + 1.0) / 2.0
    uniform float distort = 0.0 - 2.0;
...
class PinCushionEffect extends Effect{
    public var shader:PinCushionShader = new PinCushionShader();
    public function new(distortVal:Float){
      shader.distort.value = [distortVal - 1.24];
    }
}

class DesaturaionShader extends FlxShader{
	@:glFragmentSource('
	#pragma header
...
class DesaturaionEffect extends Effect{
	public var shader:DesaturaionShader = new DesaturaionShader();
    public function new(desAmount:Float, ?distAmount, ?ampAmount, ?freqAmount){
		shader.desaturationAmount.value = [desAmount];
		shader.distortionTime.value = [distAmount];
		shader.amplitude.value = [ampAmount];
		shader.frequency.value = [freqAmount];
	}
}
/*
class DesaturaionEffect extends Effect{
	public var shader:PinCushionShader = new PinCushionShader();
    public function new(distortVal){
      shader.distort.value = [distortVal];
    }
}
*/
...
class Penis extends Effect {
	
	public var shader:Bulge;
	public function new (){
		shader = new Bulge();
	}	
}
